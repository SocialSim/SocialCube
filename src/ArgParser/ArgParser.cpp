...
void ArgParser::initSocialCubeArgFromCLI(int argc, const char* argv[]) {
    try { 
        ...
        { 
            ...
            if (result.count("default_file_path")) {
                default_file_path = result["default_file_path"].as<string>();
            } else {
...
string ArgParser::getDefaultFilePath() {
...
void ArgParser::parseTime(const std::string& str_time, time_t& simulator_time) {
    ...
    if (ss >> std::get_time(&t, "%Y-%m-%dT%H:%M:%S1"))
    {
